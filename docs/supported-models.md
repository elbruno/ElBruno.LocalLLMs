...
> **Auto-download:** MagenticBrain and Fara are both ready today. Fara uses the published multimodal package generated by `scripts/convert_fara_multimodal.py`.
>
...
>
> **Fara1.5-9B** is a computer-use agent fine-tuned from Qwen3.5-9B-VL. The built-in builder still maps `Qwen3_5ForConditionalGeneration` to `Qwen35TextModel` with `exclude_embeds=true`; the published `elbruno/Fara1.5-9B-onnx` package is completed via the custom multimodal export pipeline.
>
